...
Private m_SuppressAdresseUebernahme As Boolean  ' Verhindert Adressabfrage beim programmgesteuerten Vorbelegen

Public Property Let SuppressAdresseUebernahme(ByVal value As Boolean)
    m_SuppressAdresseUebernahme = value
End Property

Public Property Get SuppressAdresseUebernahme() As Boolean
    SuppressAdresseUebernahme = m_SuppressAdresseUebernahme
End Property
...
Private Function FrageWeitereMitgliederAustritt(ByVal parzelle As String, ByVal memberID As String) As String
    ...
    If antwort = vbYes Then
        FrageWeitereMitgliederAustritt = "JA"
...
Private Function HolePachtbeginnVonMemberID(ByVal memberID As String, ByRef eintritt As Date) As Boolean
    ...
    Dim r As Long
    Dim lastRow As Long
    Dim valPB As Variant

    Set ws = ThisWorkbook.Worksheets(WS_MITGLIEDER)
    lastRow = ws.Cells(ws.Rows.count, M_COL_NACHNAME).End(xlUp).Row

    For r = M_START_ROW To lastRow
        If StrComp(Trim(CStr(ws.Cells(r, M_COL_MEMBER_ID).value)), Trim(memberID), vbTextCompare) = 0 Then
            valPB = ws.Cells(r, M_COL_PACHTANFANG).value
            If IsDate(valPB) Then
                eintritt = CDate(valPB)
                HolePachtbeginnVonMemberID = True
                Exit Function
            End If
        End If
    Next r
...
' ***************************************************************
    ...
    If m_SuppressAdresseUebernahme Then Exit Sub
...
Private Sub cmd_Entfernen_Click()
    ...
    Dim dtNachpaechterEintritt As Date
    Dim austrittsModusTag As String
    Dim tagStr As String
    Dim dlgAustritt As frm_Austrittsauswahl
    Dim rueckgabeTag As String
    Dim rueckgabeTeile() As String
    Dim neuerNachpaechterID As String
    Dim neuerNachpaechterName As String
    ...
    ' Zeige Austrittsauswahl-Dialog (eigene Instanz, robust bei Abbruch/Fenster-X)
    Set dlgAustritt = New frm_Austrittsauswahl
    With dlgAustritt
        .Show vbModal
    ...
    Select Case auswahlOption
        Case 1 ' Nachp�chter
            ...
            ' Die Frage nach weiteren Mitgliedern gehört direkt an den Austrittsbeginn.
            weitereMitgliederFlag = FrageWeitereMitgliederAustritt(OldParzelle, OldMemberID)
            If weitereMitgliederFlag = "ABBRUCH" Then
                Me.tag = lRow
                Exit Sub
            End If
            ...
            If nachpaechterID = "NACHPAECHTER_NEU" Then
                ...
                Me.tag = lRow & "|" & ChangeReason & "|NACHPAECHTER_NEU|" & OldParzelle & "|" & weitereMitgliederFlag
                ...
                With frmNachpaechter
                    ...
                    .SuppressAdresseUebernahme = True
                    ...
                    .SuppressAdresseUebernahme = False
                    
                    .Show vbModal

                    rueckgabeTag = CStr(.tag)
                End With

                If Left$(rueckgabeTag, Len("NACHPAECHTER_CREATED|")) = "NACHPAECHTER_CREATED|" Then
                    rueckgabeTeile = Split(rueckgabeTag, "|")
                    If UBound(rueckgabeTeile) >= 2 Then
                        neuerNachpaechterID = Trim$(rueckgabeTeile(1))
                        neuerNachpaechterName = Trim$(rueckgabeTeile(2))
                    End If
                Else
                    Me.tag = lRow
                    Exit Sub
                ...
                ' Aufräumen erfolgt erst nach der Rückgabe
                ...
                ' Nach R�ckkehr: Austritt mit neuem Nachp�chter direkt abschliessen
                ...
                Exit Sub
            Else
                ' Bestehender Nachp�chter: in den normalen Austrittsablauf mit Datumsbestaetigung
...
AustrittBearbeitenKomplett:
    ...
    If Trim(pachtEndeVal) <> "" Then
        Me.txt_Pachtende.value = pachtEndeVal
    Else
    ...
    MsgBox "KOMPLETT-AUSTRITT: Bitte erfassen oder best�tigen Sie das Austrittsdatum." & vbCrLf & _
           "Grund: " & ChangeReason & vbCrLf & vbCrLf & _
           "ALLE Parzellen (" & parzellenOhneVerein & ") werden abgegeben." & vbCrLf & vbCrLf & _
           "Klicken Sie danach auf '�bernehmen'.", vbInformation, "Komplett-Austritt"
    Exit Sub
...
AustrittBearbeiten:
    ...
    ' Speichere Grund/Nachp�chter tempor�r im Tag.
    austrittsModusTag = "NORMAL"
    If Trim(nachpaechterID) <> "" Then austrittsModusTag = "NACHPAECHTER_TRANSFER"

    Me.tag = lRow & "|" & ChangeReason & "|" & nachpaechterID & "|" & nachpaechterName & "|" & _
             austrittsModusTag & "|" & weitereMitgliederFlag & "|" & nachpaechterEintrittTag

    If Trim(pachtEndeVal) <> "" Then
        Me.txt_Pachtende.value = pachtEndeVal
    ElseIf Trim(nachpaechterEintrittTag) <> "" And IsDate(nachpaechterEintrittTag) Then
        Me.txt_Pachtende.value = Format(DateAdd("d", -1, CDate(nachpaechterEintrittTag)), "dd.mm.yyyy")
    Else
        Me.txt_Pachtende.value = Format(Date, "dd.mm.yyyy")
    End If
    Me.txt_Pachtende.SetFocus
    ...
    MsgBox "Bitte erfassen oder best�tigen Sie das Austrittsdatum." & vbCrLf & _
           "Grund: " & ChangeReason & vbCrLf & vbCrLf & _
           "Parzelle " & OldParzelle & " wird abgegeben." & vbCrLf & vbCrLf & _
           "Klicken Sie danach auf '�bernehmen'.", vbInformation, "Austrittsdatum"
    Exit Sub
...
Private Sub VerarbeiteAustrittNachNachpaechterErfassung(ByVal lRow As Long, ByVal parzelle As String, _
    ...
    If Trim$(forcedNachpaechterID) <> "" Then
        newMemberID = Trim$(forcedNachpaechterID)
        newMemberName = Trim$(forcedNachpaechterName)

        lastRow = wsM.Cells(wsM.Rows.count, M_COL_NACHNAME).End(xlUp).Row
        For r = M_START_ROW To lastRow
            If StrComp(Trim$(CStr(wsM.Cells(r, M_COL_MEMBER_ID).value)), newMemberID, vbTextCompare) = 0 Then
                If Trim$(newMemberName) = "" Then
                    newMemberName = wsM.Cells(r, M_COL_NACHNAME).value & ", " & wsM.Cells(r, M_COL_VORNAME).value
                End If
                If IsDate(wsM.Cells(r, M_COL_PACHTANFANG).value) Then
                    nachpaechterEintritt = CDate(wsM.Cells(r, M_COL_PACHTANFANG).value)
        ...
        Next r
    Else
        ' Fallback: Finde den neu angelegten Nachp�chter (letzte Zeile mit gleicher Parzelle)
        lastRow = wsM.Cells(wsM.Rows.count, M_COL_NACHNAME).End(xlUp).Row
    ...
    If zielRow = 0 Then
        ' Fallback: Wenn die Zeile nach Sortierung/Verschiebung nicht mehr direkt gefunden wird,
        ' suche robust per Name auf derselben Parzelle.
        lastRow = wsM.Cells(wsM.Rows.count, M_COL_NACHNAME).End(xlUp).Row
        For r = M_START_ROW To lastRow
            If StrComp(Trim$(CStr(wsM.Cells(r, M_COL_PARZELLE).value)), Trim$(parzelle), vbTextCompare) = 0 And _
               StrComp(Trim$(CStr(wsM.Cells(r, M_COL_MEMBER_ID).value)), Trim$(newMemberID), vbTextCompare) <> 0 And _
               StrComp(Trim$(CStr(wsM.Cells(r, M_COL_NACHNAME).value)), Trim$(nachname), vbTextCompare) = 0 And _
               StrComp(Trim$(CStr(wsM.Cells(r, M_COL_VORNAME).value)), Trim$(vorname), vbTextCompare) = 0 Then
                zielRow = r
                Exit For
            End If
        Next r
    ...
    ElseIf weitereMitgliederFlag = "NEIN" Then
        weitereAustritte = False
    Else
        weitereAnzahl = mod_Mitglieder_Logik.AnzahlWeitereMitgliederAufParzelle(parzelle, memberID, newMemberID)
...
Private Sub cmd_Uebernehmen_Click()
    ...
    If InStr(Me.tag, "|") > 0 Then
        ...
        If mod_Mitglieder_Logik.IsNumericTag(tagParts(0)) And UBound(tagParts) >= 1 Then
            ...
            ' Optionales Modus-Flag im 5. Segment (z.B. KOMPLETT / NACHPAECHTER_NEU)
            Dim istKomplettAustritt As Boolean
            istKomplettAustritt = False
            If UBound(tagParts) >= 4 Then
                austrittsModus = UCase(Trim(tagParts(4)))
                If austrittsModus = "KOMPLETT" Then
                    istKomplettAustritt = True
                End If
            End If
            If UBound(tagParts) >= 5 Then weitereMitgliederFlag = UCase(Trim(tagParts(5)))
            If UBound(tagParts) >= 6 Then nachpaechterEintrittTag = Trim(tagParts(6))
            ...
            Else
                ...
                Call cmd_Uebernehmen_MitAustritt(lRow, grund, nachpaechterName, nachpaechterID, austrittsModus, weitereMitgliederFlag, nachpaechterEintrittTag)
            End If
            Exit Sub
        End If
    End If
    ...
    ' MsgBox "Fehler beim Speichern der �nderungen" ausloeste.
    If IsNumeric(Me.tag) Then
        lRow = CLng(Me.tag)
    End If
    
    If lRow < 2 Then
        MsgBox "Interner Fehler: Datenzeile nicht ermittelbar (Tag=" & CStr(Me.tag) & ")." & vbCrLf & _
               "Bitte das Formular schliessen und das Mitglied erneut aus der Liste �ffnen.", _
               vbCritical, "Speichern nicht m�glich"
        Exit Sub
    End If
...
Private Sub cmd_Uebernehmen_MitAustritt(ByVal lRow As Long, ByVal grund As String, _
                                         ...
                                         Optional ByVal austrittsModus As String = "", _
                                         Optional ByVal weitereMitgliederFlag As String = "", _
                                         Optional ByVal nachpaechterEintrittTag As String = "")
    ...
    If zielRow > 0 Then
        If StrComp(Trim(wsM.Cells(zielRow, M_COL_NACHNAME).value), Trim(Me.lbl_Nachname.Caption), vbTextCompare) <> 0 Or _
           StrComp(Trim(wsM.Cells(zielRow, M_COL_VORNAME).value), Trim(Me.lbl_Vorname.Caption), vbTextCompare) <> 0 Or _
           StrComp(Trim(wsM.Cells(zielRow, M_COL_PARZELLE).value), Trim(Me.lbl_Parzelle.Caption), vbTextCompare) <> 0 Then
            zielRow = 0
        End If
    End If
    ...
    If zielRow = 0 Then
        For r = M_START_ROW To lastRow
            If StrComp(Trim(wsM.Cells(r, M_COL_NACHNAME).value), Trim(Me.lbl_Nachname.Caption), vbTextCompare) = 0 And _
               StrComp(Trim(wsM.Cells(r, M_COL_VORNAME).value), Trim(Me.lbl_Vorname.Caption), vbTextCompare) = 0 And _
               StrComp(Trim(wsM.Cells(r, M_COL_PARZELLE).value), Trim(Me.lbl_Parzelle.Caption), vbTextCompare) = 0 Then
                zielRow = r
                Exit For
            End If
        Next r
    ...
    If zielRow = 0 Then
        MsgBox "Austritt konnte nicht zugeordnet werden. Bitte Mitglied erneut �ffnen und Vorgang wiederholen.", vbCritical
        Exit Sub
    End If

    nachname = wsM.Cells(zielRow, M_COL_NACHNAME).value
    vorname = wsM.Cells(zielRow, M_COL_VORNAME).value
    OldParzelle = wsM.Cells(zielRow, M_COL_PARZELLE).value
    OldMemberID = wsM.Cells(zielRow, M_COL_MEMBER_ID).value
    ...
    If UCase(Trim(weitereMitgliederFlag)) = "JA" Then
        weitereAustritte = True
    ElseIf UCase(Trim(weitereMitgliederFlag)) = "NEIN" Then
        weitereAustritte = False
    Else
        weitereAnzahl = mod_Mitglieder_Logik.AnzahlWeitereMitgliederAufParzelle(OldParzelle, OldMemberID, nachpaechterID)
        If weitereAnzahl > 0 Then
            weitereAntwort = MsgBox("Auf Parzelle " & OldParzelle & " sind noch " & weitereAnzahl & " weitere gemeldete Person(en)." & vbCrLf & vbCrLf & _
                                    "Sollen diese ebenfalls austreten?" & vbCrLf & _
                                    "(gleicher Grund und gleicher Nachp�chter)", _
                                    vbYesNoCancel + vbQuestion, "Weitere Personen auf Parzelle")
            If weitereAntwort = vbCancel Then Exit Sub
            weitereAustritte = (weitereAntwort = vbYes)
        End If
    End If

    If Trim(nachpaechterID) <> "" And Trim(nachpaechterEintrittTag) <> "" Then
        If IsDate(nachpaechterEintrittTag) Then
            If austrittsDatum > CDate(nachpaechterEintrittTag) Then
                MsgBox "Hinweis: Austrittsdatum liegt nach dem Eintrittsdatum des Nachp" & ChrW(228) & "chters.", vbExclamation, "Datum pr" & ChrW(252) & "fen"
            End If
        End If
    End If

    grundFuerWeitere = grund
    If Trim(nachpaechterID) <> "" Then
        grundFuerWeitere = "P" & ChrW(228) & "chterwechsel"
    End If

    ' Bei Austritt mit Nachp�chter erfolgt KEIN Parzellenwechsel des Nachp�chters
    ' in diesem Ablauf. Der Nachp�chter bleibt unver�ndert, Vorp�chter gehen in Historie.
    ...
    If weitereAustritte Then
        weitereVerschoben = mod_Mitglieder_Logik.VerschiebeWeitereMitgliederAufParzelleInHistorie( _
                           OldParzelle, OldMemberID, austrittsDatum, grundFuerWeitere, nachpaechterName, nachpaechterID, nachpaechterID)
        If weitereVerschoben > 0 Then
            MsgBox weitereVerschoben & " weitere Person(en) auf Parzelle " & OldParzelle & " wurden ebenfalls ausgetragen.", vbInformation
        End If
    End If
...
Private Sub cmd_Uebernehmen_MitKomplettAustritt(ByVal lRow As Long, ByVal grund As String)
    ...
    lastRow = wsM.Cells(wsM.Rows.count, M_COL_NACHNAME).End(xlUp).Row
    If lRow >= M_START_ROW And lRow <= lastRow Then
        OldMemberID = wsM.Cells(lRow, M_COL_MEMBER_ID).value
    End If

    If Trim(OldMemberID) = "" Then
        For r = M_START_ROW To lastRow
            If StrComp(Trim(wsM.Cells(r, M_COL_NACHNAME).value), Trim(Me.lbl_Nachname.Caption), vbTextCompare) = 0 And _
               StrComp(Trim(wsM.Cells(r, M_COL_VORNAME).value), Trim(Me.lbl_Vorname.Caption), vbTextCompare) = 0 Then
                OldMemberID = wsM.Cells(r, M_COL_MEMBER_ID).value
                Exit For
            End If
        Next r
    End If
    ...
    If Trim(OldMemberID) = "" Then
        MsgBox "Mitglied f�r Komplett-Austritt konnte nicht zugeordnet werden.", vbCritical
        Exit Sub
    End If
...
Private Sub cmd_Anlegen_Click()
    ...
    If InStr(CStr(Me.tag), "NACHPAECHTER_NEU") > 0 Then
        Me.tag = "NACHPAECHTER_CREATED|" & newMemberID & "|" & _
                 Trim$(Me.txt_Nachname.value) & ", " & Trim$(Me.txt_Vorname.value)
        Me.Hide
        Exit Sub
    End If
...
Private Sub UserForm_Initialize()
    ...
    ' F�lle cbo_Parzelle OHNE "Verein"
