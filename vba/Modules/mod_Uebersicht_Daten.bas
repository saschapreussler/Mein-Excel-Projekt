...
' ZWECK: Datenquellen und Hilfsfunktionen f?r die ?bersicht
'        - Kategorien aus Einstellungen laden (inkl. F?lligkeit)
...
' I (S?umnis-Geb?hr) + F?lligkeit aus Daten Spalte O
...
Public Sub LadeKategorienAusEinstellungen(ByRef kategorien() As UebKategorie, _
    ...
    For Each key In dict.keys
        ...
        With kategorien(idx)
            ...
            ' F?lligkeit aus Daten-Blatt Spalte O (Kategorie-Tabelle)
            .faelligkeit = ""
        End With
    ...
    ' F?lligkeit aus Daten-Blatt nachladen (Spalte O)
...
Public Function HoleAktiveMitglieder(ByVal wsDaten As Worksheet) As Collection
    ...
    ' Schl?ssel: EntityKey -> Funktion (Spalte O)
    ...
    For r = EK_START_ROW To lastRow
        ...
        ' Falls in Mitgliederliste Spalte O eine ?nderung erfolgte
...
' vorkommt. Gibt 0 zur?ck wenn keine Daten vorhanden.
' ===============================================================
...
' f?r jeden Monat der mindestens eine Buchung enth?lt
' Gibt Boolean-Array(1 To 12) zur?ck
' ===============================================================
...
' Zweck: Dezember-Zahlungen die f?r Januar gelten erkennen
' ===============================================================
Public Sub BefuelleVorjahrSpeicher(ByVal vorjahr As Long)
    ...
    For r = BK_START_ROW To lastRowBK
        ...
        ' EntityKey via IBAN aufloesen (?ber EntityKey-Tabelle)
        Dim vjEK As String
        vjEK = ""
        Dim ek As Long
...
' ===============================================================
' Pr?ft automatisch ob Vorjahr-Speicher gel?scht werden soll
...
Public Sub PruefeVorjahrSpeicherAblauf()
    ...
    If Month(Date) >= 8 Then
        ...
        ' Pr?fen ob noch Daten im Speicher sind
        ...
        If IsDate(ersteDatum) Then
            If Year(CDate(ersteDatum)) < Year(Date) - 1 Then
                ' Daten sind aelter als Vorjahr -> l?schen
                Call LoescheVorjahrSpeicher
            ElseIf Year(CDate(ersteDatum)) = Year(Date) - 1 Then
                ' Vorjahr-Daten und wir sind >= August -> l?schen
                Call LoescheVorjahrSpeicher
            End If
        End If
    End If
...
' Pr?ft ob f?r den EntityKey + Kategorie eine Dezember-Zahlung
' vorliegt, die f?r Januar des Folgejahres gelten k?nnte
...
Public Function HoleVorjahrZahlung(ByVal entityKey As String, _
    ...
    ' Nur f?r fruehe Monate relevant (Jan-Maerz)
...
' ===============================================================
' v4.6: Pr?ft ob Vorjahr-Daten im Speicher vorhanden sind
' Gibt True zur?ck wenn mindestens eine Zeile in CA-CF existiert
' ===============================================================
...
' Gibt Collection von Dictionaries zur?ck:
