...
'   - OnKategorieChange: Kategorie-?nderung Validierung
'   - ZentriereAlleZellenVertikal: Vertikale Zentrierung
'   - FormatiereNeuesBlatt: Neues Blatt formatieren
' ***************************************************************

' ===============================================================
' Zentriert ALLE Zellen auf ALLEN Bl?ttern vertikal
' ===============================================================
...
' relevanten Bl?ttern vorhanden und nutzbar sind.
...
Public Sub StelleAutoFilterBereit()
    ...
    For i = LBound(cfg) To UBound(cfg)
        ...
        ' Blatt mit AllowFiltering sch?tzen
        On Error Resume Next
...
NextSheet:
    ...
    ' Zur?ck zur Startseite navigieren
...
' ===============================================================
' Validierung und Reaktion bei ?nderung in Kategorie-Tabelle
' ===============================================================
Public Sub OnKategorieChange(ByVal Target As Range)
    ...
    ' Spalte J oder K ge?ndert: Dropdown-Listen aktualisieren
    If Target.Column = DATA_CAT_COL_KATEGORIE Or Target.Column = DATA_CAT_COL_EINAUS Then
        Call mod_Format_Dropdowns.AktualisiereKategorieDropdownListen(ws)
    End If
    
    ' Spalte K (E/A) ge?ndert: Konsistenzpr?fung
    ...
    ' Spalte L (Keyword) ge?ndert: Duplikat-Pr?fung
...
' ===============================================================
' WORKSHEET_CHANGE HANDLER f?r dynamische Formatierung
' ===============================================================
...
' ---------------------------------------------------------------
' Sorgt f?r ein durchgehend konsistentes Layout der
' Mitgliederliste, immer wenn sich Daten ?ndern. Wird gerufen von:
'   - Workbook_Open
'   - Worksheet_Change auf Mitgliederliste (Tabelle7)
'   - mod_Mitglieder_UI nach Speichern / L?schen / Sortieren
'
' Setzt:
...
'   - Datumsformat f?r Geburtstag / Pacht-Spalten
'
' Wird intern stets unter ScreenUpdating=False und EnableEvents=False
' ausgef?hrt, damit es keine Reentrancy in Worksheet_Change gibt.
' ===============================================================
