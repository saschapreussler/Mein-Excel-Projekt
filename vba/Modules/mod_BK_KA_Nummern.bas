...
' ZWECK: Punkt 11 - Vergibt fortlaufende Nummern fuer
'        Bankkonto-Ausgaben (BK NN) und Vereinskassen-Eintraege (KA NN).
'        Bei Bargeldauszahlung wird zusaetzlich ein VK-Eintrag mit
'        Bezug zur BK-Nummer angelegt.
'
' Schema (Spalte J auf Bankkonto, Spalte F auf Vereinskasse):
'   - "BK 01", "BK 02", ... fuer Ausgaben (Betrag < 0) je Jahr
'   - "KA 01", "KA 02", ... fuer Vereinskassen-Eintraege je Jahr
...
' ===============================================================
' Public: Vergibt BK-Nummer fuer eine einzelne Bankkonto-Zeile.
...
' ===============================================================
' Public: Neuberechnung aller BK/KA-Nummern fuer das aktuelle
' Abrechnungsjahr. Wird nach CSV-Import oder bei Aenderung der
...
Public Sub NeuberechneAlleBKNummern(Optional ByVal wsBK As Worksheet = Nothing)
    ...
    ' --- Schritt 1: BK-Spalte J fuer das Abrechnungsjahr leeren ---
    '     (Eintraege aus Vorjahren bleiben unangetastet)
    Dim abrJahr As Long
...
NumeriereKA:
    ' --- Schritt 4: VK-Eintraege fuer Bargeldauszahlung sicherstellen + KA-Nummern vergeben ---
...
' - Fuer jede Bankkonto-Zeile mit Kategorie "Bargeldauszahlung" und
'   BK-Nummer im aktuellen Jahr wird ein VK-Eintrag sichergestellt
'   (anhand von Datum + Betrag identifiziert).
' - Bestehende VK-Eintraege werden NICHT dupliziert.
' ===============================================================
Private Sub SyncBargeldauszahlungenZuVK(ByVal wsBK As Worksheet, _
    ...
    For r = BK_START_ROW To lastBK
        ...
        ' Falls der Eintrag noch keine BK-Nr hat, ueberspringen (kommt in naechstem Lauf)
        ...
        ' Pruefen ob VK-Eintrag bereits existiert (Datum + Betrag positiv)
                ...
                If CDate(wsVK.Cells(v, VK_COL_DATUM).value) = bkDatum Then
                    If IsNumeric(wsVK.Cells(v, VK_COL_BETRAG).value) Then
                        If Abs(CDbl(wsVK.Cells(v, VK_COL_BETRAG).value) - Abs(bkBetrag)) < 0.005 Then
                            ' Zusaetzlich pruefen ob Beschreibung auf Bargeldauszahlung hindeutet
                            Dim besch As String
...
' ===============================================================
' Numeriert alle VK-Eintraege im Abrechnungsjahr nach Datum
