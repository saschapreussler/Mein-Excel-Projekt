...
'               enthaelt: Parzellen-Pruefungen, Historie-Operationen,
...
' --- Hilfsfunktion ffuer Parzelle -> Seite ---
...
' --- Hilfsfunktion: Pruefe ob String eine Zahl ist ---
...
' --- Pruefe ob FormularForm geladen ist ---
...
' ***************************************************************
' prueft ob Person bereits auf dieser Parzelle existiert
' ***************************************************************
...
' ***************************************************************
' prueft ob auf einer Parzelle noch zahlende Mitglieder sind
' ***************************************************************
Public Function HatParzelleNochZahlendesMitglied(ByVal parzelle As String, ByVal ausschlussMemberID As String) As Boolean
    ...
    For r = M_START_ROW To lastRow
        If StrComp(Trim(ws.Cells(r, M_COL_PARZELLE).value), parzelle, vbTextCompare) = 0 Then
            ...
            ' Ignoriere die auszuschliessende Member-ID
            If memberID <> ausschlussMemberID Then
                ' Pruefe ob zahlendes Mitglied
...
' ***************************************************************
' prueft ob eine Parzelle zahlendes Mitglied hat
' ***************************************************************
...
' ***************************************************************
' prueft ob Person auf Parzelle existiert
' ***************************************************************
...
' ***************************************************************
' prueft ob Parzelle leer ist (keine aktiven Mitglieder)
' ***************************************************************
...
' ***************************************************************
' Zeigt Auswahl-Dialog ffuer mehrere Mitglieder auf einer Parzelle
' ***************************************************************
...
Public Sub VerschiebeInHistorie(ByVal lRow As Long, ByVal parzelle As String, ByVal memberID As String, _
    ...
    ' === SICHERHEITSCHECK: NIEMALS Verein-Parzelle Loeschen ===
    If UCase(Trim(parzelle)) = "VEREIN" Then
    ...
    ' Entsperre beide Blaetter
    wsM.Unprotect PASSWORD:=PASSWORD
    wsH.Unprotect PASSWORD:=PASSWORD
    
    ' Finde NAECHSTE freie Zeile in Mitgliederhistorie (ab Zeile 4)
    ...
    ' Loesche Zeile aus Mitgliederliste
    wsM.Rows(lRow).Delete Shift:=xlUp
    
    ' Schuetze Blaetter wieder
...
' ***************************************************************
' Verschiebt ALLE Eintraege eines Mitglieds (alle Parzellen)
...
Public Sub VerschiebeAlleParzellenInHistorie(ByVal memberID As String, _
    ...
    ' RueckWAERTS durchlaufen wegen Zeilen-Loeschung!
    For r = lastRow To M_START_ROW Step -1
        If wsM.Cells(r, M_COL_MEMBER_ID).value = memberID Then
            ...
            ' Finde NAECHSTE freie Zeile in Mitgliederhistorie
            nextHistRow = wsH.Cells(wsH.Rows.count, H_COL_NAME_EHEM_PAECHTER).End(xlUp).Row + 1
            If nextHistRow < H_START_ROW Then nextHistRow = H_START_ROW
