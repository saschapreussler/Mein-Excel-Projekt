...
Public Sub Aktualisiere_Parzellen_Mapping_Final()
    ...
    For rD = DATA_START_ROW To lastRowD
        ...
        If Trim(wsD.Cells(rD, DATA_MAP_COL_ZUORDNUNG).Value) <> "" Then
            wsD.Cells(rD, DATA_MAP_COL_DEBUG).Value = "Manuell zugeordnet oder best�tigt"
...
Private Sub ApplyMappingTableFormatting(ByVal ws As Worksheet, ByVal lastDataRow As Long)
    ...
    ' Zeilenh�he NACH Spalte-U-Korrektur nochmals sauber setzen
    ws.Rows(DATA_START_ROW & ":" & lastDataRow).AutoFit
    ...
    With ddRange.Validation
        ...
        .IgnoreBlank = True
        .InCellDropdown = True
        .InputTitle = "Rolle w�hlen"
        .ErrorTitle = "Ung�ltige Rolle"
        .ErrorMessage = "Bitte w�hlen Sie eine g�ltige Rolle aus der Liste."
    End With
...
Public Sub Importiere_Kontoauszug()
    ...
    For i = BK_START_ROW To lRowZiel
        If wsZiel.Cells(i, BK_COL_BETRAG).Value <> "" Then
            ' Schl�ssel: Datum | Betrag | IBAN | Verwendungszweck
    ...
    ' Improved: Auto-detect delimiter and handle UTF-8 BOM
    With wsTemp.QueryTables.Add(Connection:="TEXT;" & strFile, Destination:=wsTemp.Cells(1, 1))
        ...
        ' Enable multiple delimiters for flexible detection
    ...
    ' Skip blank lines by finding actual last row with data
    Dim checkRow As Long
    lastRowTemp = wsTemp.Cells(wsTemp.Rows.Count, 1).End(xlUp).Row
    
    ' Remove trailing blank rows
    For checkRow = lastRowTemp To 2 Step -1
        If Application.WorksheetFunction.CountA(wsTemp.Rows(checkRow)) = 0 Then
            lastRowTemp = checkRow - 1
        Else
            Exit For
        End If
    Next checkRow
    ...
    ' Improved: Skip blank lines and handle edge cases
    
    For lRowTemp = 2 To lastRowTemp
        
        ' Skip blank rows (additional safety check)
        If Application.WorksheetFunction.CountA(wsTemp.Rows(lRowTemp)) = 0 Then
            GoTo NextRow
        End If
        ...
        ' Duplikatspr�fung mit dem stabilen Schl�ssel
...
ImportEnde:
    ...
    ' Die Filtereintr�ge werden als Fehler im Protokoll gewertet
    ...
    ' 7. Tempor�res Blatt l�schen
    If Not wsTemp Is Nothing Then
        wsTemp.Delete
        Set wsTemp = Nothing
    End If
    
    ' 8. Sortieren und Oberfl�che aktualisieren
    ...
    If rowsTotalInFile > 0 And rowsProcessed = 0 And rowsIgnoredDupe = rowsTotalInFile And rowsFailedImport = 0 Then
          MsgBox "Achtung: Die ausgew�hlte CSV-Datei enth�lt ausschlie�lich Eintr�ge, die bereits in der Datenbank vorhanden sind (" & rowsIgnoredDupe & " Duplikate). Es wurden keine neuen Datens�tze importiert.", vbExclamation, "100% Duplikate erkannt"
    ElseIf rowsProcessed > 0 Then
        MsgBox "Import abgeschlossen! (" & rowsProcessed & " neue Zeilen hinzugef�gt)", vbInformation
    End If
...
SortError:
    MsgBox "Achtung: Die automatische Sortierung konnte nicht durchgef�hrt werden! Bitte pr�fen Sie das Tabellenblatt '" & WS_BANKKONTO & "' manuell auf ung�ltige Datumswerte. Fehler: " & Err.Description, vbCritical
...
' Stellt sicher, dass das tempor�re Protokoll-Blatt existiert und gibt es zur�ck
Private Function Get_Protocol_Temp_Sheet() As Worksheet
    ...
    If Get_Protocol_Temp_Sheet Is Nothing Then
        ' F�gt das Blatt nach dem letzten Blatt ein
...
Public Sub Initialize_ImportReport_ListBox()
    ...
    ' Nur initialisieren, wenn die Historie leer ist (d.h. noch kein Import durchgef�hrt)
    If CStr(wsDaten.Range(CELL_IMPORT_PROTOKOLL).Value) = "" Then
        
        ' 1. Tempor�ren Bereich leeren
        wsTemp.Cells.ClearContents
        
        ' 2. Standardtext in den tempor�ren Bereich schreiben
        wsTemp.Range(PROTOCOL_RANGE_START).Value = "--------------------------------------------------------"
        wsTemp.Range(PROTOCOL_RANGE_START).Offset(1, 0).Value = " Kein Import-Bericht verf�gbar."
        wsTemp.Range(PROTOCOL_RANGE_START).Offset(2, 0).Value = " F�hren Sie einen CSV-Import durch, um den Bericht hier anzuzeigen."
        wsTemp.Range(PROTOCOL_RANGE_START).Offset(3, 0).Value = "--------------------------------------------------------"
        
        ' 3. ListBox mit dem tempor�ren Bereich verkn�pfen
        On Error Resume Next
...
Public Sub Update_ImportReport_ListBox(ByVal totalEntries As Long, ByVal importedEntries As Long, ByVal duplicateEntries As Long, ByVal errorEntries As Long)
    ...
    Dim wsDaten As Worksheet
    Dim wsTemp As Worksheet
    Dim protocolRange As String ' Der Bereich, der die ListBox f�llt
    ...
    ' Konfiguration f�r die Speicherung der Historie
    ...
    ' --- I. Historie Speichern und K�rzen ---
    
    ' BERICHTSTEILE F�R DIE SPEICHERUNG
    ...
    ' Historie auf maximale Gr��e trimmen und speichern
    ...
    ' --- II. Anzeige im Tempor�ren Blatt und Verkn�pfung ---
    
    wsTemp.Cells.ClearContents ' Vorherige Anzeige l�schen
    k = 1 ' Zeilenz�hler f�r das tempor�re Blatt
    ...
    For i = 0 To UBound(currentHistory)
        ...
        ' Begrenzung der Zeilen f�r die ListBox (aus Performancegr�nden)
        If k >= MAX_LISTBOX_LINES Then Exit For
    Next i
    
    ' 3. ListBox verkn�pfen
    On Error Resume Next
    ' Pr�ft, ob das Form Control existiert und verkn�pft es mit dem gef�llten Range
